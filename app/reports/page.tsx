...
export default function ReportsPage() {
  const { projects, materialRequests, siteReports, deliveries, returns, currentUser } = useStore(); const router = useRouter()
  ...
  return <div className="flex flex-1 flex-col gap-6 p-4 md:p-6">
    ...
    <div className="grid grid-cols-2 gap-4 xl:grid-cols-6"><StatsCard label="Obras activas" value={active} icon={FolderKanban} hint={`${projects.length} obras registradas`} /><StatsCard label="Solicitudes" value={materialRequests.length} icon={Layers} hint={`${materialRequests.filter(item => item.status === "pending").length} pendientes`} /><StatsCard label="Actas tecnicas" value={siteReports.length} icon={MessageSquareText} hint="recibidas desde obra" /><StatsCard label="Capacidad" value={`${projects.reduce((total, project) => total + project.capacityKw, 0)} kW`} icon={FileCheck2} hint="capacidad planeada" /><StatsCard label="Entregas retrasadas" value={deliveries.filter(item => item.status === "delayed").length} icon={Layers} hint="requieren seguimiento" /><StatsCard label="Devoluciones abiertas" value={returns.filter(item => item.status !== "closed").length} icon={MessageSquareText} hint="pendientes de cierre" /></div>
